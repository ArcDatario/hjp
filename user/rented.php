...
if (isset($_SESSION['user_id']) && !empty($_SESSION['user_id'])) {
    $user_id = intval($_SESSION['user_id']); // Sanitize the user ID

    // Prepare the SQL query
    $sql = "SELECT rental.approved_date, rental.paid_date, rental.completed_date, rental.id AS rent_id, 
                   equipment_tbl.equipment, equipment_tbl.image, rental.rent_start_date, rental.rent_end_date, 
                   rental.summary, rental.total, rental.status, users_acc.user_name, rental.downpayment, rental.equipment_id,
                   rental.first_total, rental.gcash_receipt
            FROM rental
...
?>
                <tr>
                    <td data-label="Image">
                        <img src="../admin/functions/equipment_images/<?php echo htmlspecialchars($row['image']); ?>" 
                             alt="Equipment Image" 
                             height="60" 
                             width="80" 
                    ...
                    <td data-label="Equipment"><?php echo htmlspecialchars($row['equipment']); ?></td>
                    ...
                    <td data-label="Duration"><?php echo htmlspecialchars($row['summary']); ?></td>
                    ...
                    <td data-label="Balance"><?php 
                        echo ($row['status'] === 'Paid' || $row['status'] === 'Completed') ? '0' : htmlspecialchars($row['first_total']);
                    ?></td>
                    <td data-label="Receipt">
                        <img src="gcash/<?php echo htmlspecialchars($row['gcash_receipt']); ?>" 
                             alt="Receipt Image" 
                             id="receipt" 
        ...
        <a href="view_equipment.php?id=<?php echo $row['equipment_id']; ?>&user_id=<?php echo $user_id; ?>">
            Rate
        </a>
    <?php endif; ?>
                    </td>
...
<?php
        ...
        echo "<tr><td colspan='12'>Error preparing the query.</td></tr>";
    }
} else {
    echo "<tr><td colspan='12'>User is not logged in.</td></tr>";
}
...
function formatDate($dateString) {
    if ($dateString == "0000-00-00" || empty($dateString)) {
        ...
        return (date("H:i:s", $date) === "00:00:00") ? date("M j, Y", $date) : date("M j, Y - h a", $date);
    }
}
?>
</tbody>
