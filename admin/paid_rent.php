...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $updateEquipmentSql = "UPDATE rental SET status = 'Paid', paid_status = 'Paid',  paid_date = NOW() WHERE id = $equipmentId ";
    ...
    if ($conn->query($updateEquipmentSql) === TRUE) {
        ...
        $getEmailSql = "SELECT rental.id as rent_id,rental.user_email, rental.summary, rental.total, rental.rent_start_date, rental.rent_end_date, equipment_tbl.image ,equipment_tbl.equipment, rental.downpayment , rental.first_total
                FROM rental 
        ...
        if ($result->num_rows > 0) {
            ...
            $equipment_total = $row['first_total'];
            ...
            $mail->Subject = "Your Rent has been Paid";
            $mail->Body = "     
                            <h1 style='color:green;'>✔</h1>
                            <h2>Youve just paid the remaining balance(₱$equipment_total). Rent #$equipmentId</h2>
