<?php
  ...
  $query_current_month = "SELECT IFNULL(SUM(total), 0) as total_revenue_current_month FROM orders WHERE status = 'Completed' AND YEAR(completed_date) = '$current_year' AND MONTH(completed_date) = '$current_month'";
  ...
  $query_previous_month = "SELECT IFNULL(SUM(total), 0) as total_revenue_previous_month FROM orders WHERE status = 'Completed' AND YEAR(completed_date) = '$current_year' AND MONTH(completed_date) = '$previous_month'";
  ...
  $percentage_html = '<span class="' . $class . ' small pt-1 fw-bold" style="color:black; float:right; margin-top:9%; font-size:0.8rem;" id="orders_percentage_month">'. $sign . abs($percentage_change) . '%</span>';
 
  $total_revenue_html = '<h5 id="orders_sales_month" style="font-size:1.1rem;">₱' . number_format($total_revenue_current_month, 0) . '</h5>';
