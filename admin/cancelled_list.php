	...
	<div class="sidebar transition overlay-scrollbars animate__animated  animate__slideInLeft">
        <div class="sidebar-content"> 
            ...
            <span style='margin-left:7%;' class='badge bg-danger notif' id='paidCount'></span>
				...
				<li>
					<a href="cancelled_list.php" class="active">
					<i class='bx bx-list-ul icon'></i>
						Cancelled Lists
					</a>
				</li>
...
$sql = "SELECT rental.user_id, rental.id, equipment_tbl.equipment, rental.rent_start_date, rental.rent_end_date, rental.summary, rental.total, rental.status, users_acc.user_name
FROM rental
INNER JOIN equipment_tbl ON rental.equipment_id = equipment_tbl.id
INNER JOIN users_acc ON rental.user_id = users_acc.id Where status='Cancelled'";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        <tr>
            ...
            <button class="cancel_button" onclick="undoCancel(<?php echo $row['id']; ?>)"></button>
...
function undoCancel(equipmentId) {
    // Display SweetAlert2 confirmation modal
    Swal.fire({
        title: 'Are you sure?',
        text: 'You are about to remove this rent. #' + equipmentId + ' from cancelled lists and it will go on Pending Lists' ,
        icon: 'warning',
...
Toast.fire({
  icon: "info",
  title: "Rent #"+ equipmentId  + " has go on Pending Lists"
})
